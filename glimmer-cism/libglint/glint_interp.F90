...
!
    ...
    integer,optional :: seg_verb              ! SEG Flag to output values
...
!+seg
    ...
    write(stdout,*) 'In interp_to_local (SEG) '  !SEG
...
!+seg
                   ...
                   if (present(seg_verb)) then
                     if (seg_cnt == 0 ) then
                       seg_cnt2(1)=seg_cnt2(1)+1
                     else if (seg_cnt == 1) then
                       seg_cnt2(2)=seg_cnt2(2)+1
                     else if (seg_cnt == 2) then
                       seg_cnt2(3)=seg_cnt2(3)+1
                     else if (seg_cnt == 3) then
                       seg_cnt2(4)=seg_cnt2(4)+1
                     else if (seg_cnt == 4) then
                       seg_cnt2(5)=seg_cnt2(5)+1
                     else
                       seg_cnt2(6)=seg_cnt2(6)+1
                     endif
                   endif
    ...
    if (present(seg_verb)) then
      write(stdout,'(a,7(i7,1X))') 'Interp count 2 (SEG) ',seg_verb,seg_cnt2   !SEG
    endif
  ...
  subroutine mean_to_global_mec(ups,                &
    ...
    write(stdout,*) ' RSS2  range(ltopo) = ', minval(ltopo) , maxval(ltopo)
    write(stdout,*) ' RSS2  range(ltopo_f) = ', minval(ltopo_fulldomain) , maxval(ltopo_fulldomain)
    ...
    if (main_task) then
       ...
       !write(stdout,*) "RSS topo"
       !do j = 1, nyl_full
       !   do i = 1, nxl_full
       !     write(stdout,*) i,j,ltopo_fulldomain(i,j),tempmask_fulldomain(i,j),local_fulldomain(i,j)
       !   enddo
       !enddo
       !write(stdout,*) "RSS ----"
