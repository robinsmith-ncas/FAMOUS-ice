...
contains
  ...
  subroutine glide_io_create(outfile,model,data)
    ...
    if (pos.ne.0 .and. status.eq.nf90_enotvar) then
       ...
       status = parallel_put_att(NCO%id, varid, 'units', 'pascal/year')
       if (glimmap_allocated(model%projection)) then
          status = parallel_put_att(NCO%id, varid, 'grid_mapping',glimmer_nc_mapvarname)
          status = parallel_put_att(NCO%id, varid, 'coordinates', 'lon lat')
       end if
    ...
    !     gline_flux -- grounding line flux
    ...
    if (pos.ne.0 .and. status.eq.nf90_enotvar) then
       ...
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_put_att(NCO%id, varid, 'long_name', 'grounding line flux')
       status = parallel_put_att(NCO%id, varid, 'units', 'meter2/year')
       if (glimmap_allocated(model%projection)) then
          status = parallel_put_att(NCO%id, varid, 'grid_mapping',glimmer_nc_mapvarname)
          status = parallel_put_att(NCO%id, varid, 'coordinates', 'lon lat')
    ...
    status = parallel_inq_varid(NCO%id,'init_phaml',varid)
    if (pos.ne.0) then
      NCO%vars(pos+1:pos+10) = '          '
    end if
    if (pos.ne.0 .and. status.eq.nf90_enotvar) then
       call write_log('Creating variable init_phaml')
       status = parallel_def_var(NCO%id,'init_phaml',get_xtype(outfile,NF90_FLOAT),(/x1_dimid, y1_dimid, time_dimid/),varid)
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_put_att(NCO%id, varid, 'long_name', 'phaml initial conditions')
       status = parallel_put_att(NCO%id, varid, 'standard_name', 'phaml_initial_conditions')
       status = parallel_put_att(NCO%id, varid, 'units', 'meter')
       if (glimmap_allocated(model%projection)) then
          status = parallel_put_att(NCO%id, varid, 'grid_mapping',glimmer_nc_mapvarname)
          status = parallel_put_att(NCO%id, varid, 'coordinates', 'lon lat')
    ...
    status = parallel_inq_varid(NCO%id,'phaml',varid)
    if (pos.ne.0) then
      NCO%vars(pos+1:pos+5) = '     '
    end if
    if (pos.ne.0 .and. status.eq.nf90_enotvar) then
       call write_log('Creating variable phaml')
       status = parallel_def_var(NCO%id,'phaml',get_xtype(outfile,NF90_FLOAT),(/x1_dimid, y1_dimid, time_dimid/),varid)
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_put_att(NCO%id, varid, 'long_name', 'phaml true solution')
       status = parallel_put_att(NCO%id, varid, 'standard_name', 'phaml_solution')
       status = parallel_put_att(NCO%id, varid, 'units', 'meter')
       if (glimmap_allocated(model%projection)) then
          status = parallel_put_att(NCO%id, varid, 'grid_mapping',glimmer_nc_mapvarname)
          status = parallel_put_att(NCO%id, varid, 'coordinates', 'lon lat')
    ...
    if (pos.ne.0 .and. status.eq.nf90_enotvar) then
       ...
       status = parallel_def_var(NCO%id,'uvel_icegrid',get_xtype(outfile,NF90_FLOAT),(/x1_dimid, y1_dimid, level_dimid, time_dimid/),varid)
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_put_att(NCO%id, varid, 'scale_factor',(scale_uvel))
       status = parallel_put_att(NCO%id, varid, 'long_name', 'ice velocity in x direction')
       status = parallel_put_att(NCO%id, varid, 'standard_name', 'land_ice_x_velocity')
    ...
    if (pos.ne.0 .and. status.eq.nf90_enotvar) then
       ...
       status = parallel_def_var(NCO%id,'vvel_icegrid',get_xtype(outfile,NF90_FLOAT),(/x1_dimid, y1_dimid, level_dimid, time_dimid/),varid)
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_put_att(NCO%id, varid, 'scale_factor',(scale_uvel))
       status = parallel_put_att(NCO%id, varid, 'long_name', 'ice velocity in y direction')
       status = parallel_put_att(NCO%id, varid, 'standard_name', 'land_ice_y_velocity')
  ...
  subroutine glide_io_write(outfile,data)
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%stress%tau_x, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%stress%tau_y, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
  ...
  subroutine glide_io_read(infile,data)
    ...
    status = parallel_inq_varid(NCI%id,'init_phaml',varid)
    if (status .eq. nf90_noerr) then
    ...
    status = parallel_inq_varid(NCI%id,'phaml',varid)
    if (status .eq. nf90_noerr) then
  ...
  subroutine glide_get_taux(data,outarray)
    ...
    outarray = (1e-3*thk0*thk0/len0)*(data%stress%tau_x)
  ...
  subroutine glide_set_taux(data,inarray)
    ...
    data%stress%tau_x = inarray/(1e-3*thk0*thk0/len0)
  ...
  subroutine glide_get_tauy(data,outarray)
    ...
    outarray = (1e-3*thk0*thk0/len0)*(data%stress%tau_y)
  ...
  subroutine glide_set_tauy(data,inarray)
    ...
    data%stress%tau_y = inarray/(1e-3*thk0*thk0/len0)
