...
*/
*/ Allow archiving
*/ copied from /net/jasmin/users/simon/um64/mods/scripts/archive.mod 20.5.18
*/ and modified for new ~jeff
*/
*DECLARE qsmain
*I qsmain.653 
  rm $PIPE
  ...
  then
    ...
    cp $MODEL_FILE $DEST_DATA
	  ...
	  mv ${DEST_DATA}.pp ${DEST_DATA}.${timestamp}.pp
        ...
        echo copy2dest: $MODEL_FILE
        ff2pp $MODEL_FILE ${DEST_DATA}.tmp.pp # > /dev/null
	bigend -32 ${DEST_DATA}.tmp.pp ${DEST_DATA}.pp
        if [[ ! -s ${DEST_DATA}.pp ]]
          then ls -l $MODEL_FILE ${DEST_DATA}.tmp.pp ${DEST_DATA}.pp 2>&1
          mv $MODEL_FILE keep_${MODEL_FILE}
          echo copy2dest: archive failure
        fi
