...
module glint_global_grid
  ...
  type global_grid
     ...
     integer :: nmsk = 1 !*FD Number of 3D masks  SEG 28/2/17
     ...
     integer,pointer,dimension(:,:,:,:) :: mask3D => null()
     ! SEG 28/2/17  3D mask
...
!  subroutine new_global_grid(grid, lons, lats, lonb, latb, radius, correct, nec, mask,mask3D)
! Added mask3D and nmsk
  subroutine new_global_grid(grid, lons, lats, lonb, latb, radius, correct, nec, nmsk,mask,mask3D)
!  subroutine new_global_grid(grid, lons, lats, lonb, latb, radius, correct, nec, nmsk)
    ...
    integer,               optional,intent(in)    :: nmsk    !*FD  Number of 3D masks SEG 28/2/17
    integer,dimension(:,:),optional,intent(in)    :: mask    !*FD  Mask indicating where global data are valid 
    integer,dimension(:,:,:,:),optional,intent(in)  :: mask3D    !*FD  Mask indicating where global data are valid  SEG 28/2/17
    ...
    if (associated(grid%mask3D))      deallocate(grid%mask3D) ! SEG 27/7/17
    ...
    ! This code moved from later and allocate added for mask3D  SEG 27/2/17
    if (present(nec) .and. present(nmsk)) then
       grid%nec  = nec
       grid%nmsk = nmsk
       print*,'Allocating space for 3D mask'
       allocate(grid%mask3D(grid%nmsk,grid%nx,grid%ny,grid%nec)) ! SEG 28/2/17 Added nmsk dimension
    else
       grid%nec = 1
    endif 
...
!+ SEG 1/3/17
    if (present(mask3D)) then
       ...
       grid%mask3D(:,:,:,:) = 1   ! assume global data are valid everywhere 
    endif
!- SEG 1/3/17



!! SEG 27/2/17 code moved to earlier in routine
!!    ! Set number of elevation classes
!!
!!    if (present(nec)) then
!!       grid%nec = nec
...
!!    endif 
  ...
  subroutine copy_global_grid(in,out)
    ...
    if (associated(out%mask3D))      deallocate(out%mask3D)  ! SEG 27/2/17
    ...
    allocate(out%mask3D(out%nmsk,out%nx,out%ny,out%nec)) ! SEG 27-28/2/17
    ...
    out%mask     =in%mask
    out%mask3D   =in%mask3D  ! SEG 27/2/17

  end subroutine copy_global_grid

  !-----------------------------------------------------------------------------

  subroutine get_grid_dims(grid,nx,ny,nec,nmsk) ! SEG 28/2/17 added nmsk

    type(global_grid),intent(in)  :: grid
    integer,intent(out) :: nx,ny
    integer,intent(out), optional :: nec
    integer,intent(out), optional :: nmsk ! SEG 28/2/17 added nmsk

    nx=grid%nx
    ny=grid%ny

    if (present(nec)) nec=grid%nec
    if (present(nmsk)) nmsk=grid%nmsk  ! SEG 28/2/17 added nmsk
