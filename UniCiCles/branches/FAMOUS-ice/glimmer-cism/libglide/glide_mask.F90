...
!       Probably should loop over locally owned cells only, then do a halo update.
    ...
    ! Fudge for inland sea. SEG 18/4/18
    ! This should be an option (or done better)a
    ! Points we want to be ocean have big negative topog
    !where (topg - eus < con * thck)
    write(*,*) 'glide_mask: crude mapping of ocean/inland sea'
    where (topg < -1000)
