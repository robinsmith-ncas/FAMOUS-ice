...
c use if diagnostics required are u.grad(T)    
          x_div(i,k)=(f_adv(i,k)-f_adv(i+1,k))          
     &                                       *dxtr(i)*recip_cos(j) 
          x_div(i,k)=0.5*x_div(i,k)*(fuw(i,k)+fuw(i+1,k))          
c use if diagnostics required are div(uT)                               
c          x_div(i,k)=(flux(i,k)-flux(i+1,k))*dxtr(i)*recip_cos(j) 
        enddo ! over i                                                  
      enddo ! over k                                                    
      endif !l_xadv
...
c use if diagnostics required are u.grad(T)    
!!!!!!!!!!!!!!   is this still valid ?? !!!!!!!!!!!!!!                         
          if(fvs(i,k).ne.0.0) then                                      
            y_div(i,k)=(fluxs(i,k)/fvs(i,k)-f_adv(i,k))*dytr(j)     
          else                                                          
            y_div(i,k)=-f_adv(i,k)*dytr(j)                          
          endif                                                         
          y_div(i,k)=0.5*y_div(i,k)*(fvs(i,k)+fvn(i,k)) 
c use if diagnostics required are div(uT)                               
c          y_div(i,k)=(fluxs(i,k)-fluxn(i,k))*dytr(j)                    
c          source(i,k)=source(i,k)+y_div(i,k)                           
!          h_div(i,k)=source(i,k)                                        
        enddo ! over i                                                  
      enddo ! over k                                                    
      endif ! l_yadv
...
c use if diagnostics required are u.grad(T)    
!!!!!!!!!!!!!!   is this still valid ?? !!!!!!!!!!!!!!                         
          z_div(i,k)=(f_adv(i,k+1)-f_adv(i,k))*dzr(k)   
          z_div(i,k)=0.5*z_div(i,k)*(w(i,k)+w(i,k+1))             
c use if diagnostics required are div(uT)                               
c          y_div(i,k)=(fluxs(i,k)-fluxn(i,k))*dytr(j)                    
c          source(i,k)=source(i,k)+y_div(i,k)                           
!          h_div(i,k)=source(i,k)                                        
